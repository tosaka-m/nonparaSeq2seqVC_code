...
class Decoder(nn.Module):
    def __init__(self,
        ...
        self.n_frames_per_step = n_frames_per_step_decoder
        self.hidden_cat_dim = encoder_embedding_dim + speaker_embedding_dim
        self.attention_rnn_dim = attention_rnn_dim
        self.decoder_rnn_dim = decoder_rnn_dim
        self.prenet_dim = prenet_dim
        self.max_decoder_steps = max_decoder_steps
        self.stop_threshold = stop_threshold
        self.feed_back_last = feed_back_last

        if feed_back_last:
            prenet_input_dim = n_mel_channels
        else:
            prenet_input_dim = n_mel_channels * n_frames_per_step_decoder
        ...
        self.attention_rnn = nn.LSTMCell(
            prenet_dim[-1] + self.hidden_cat_dim,
            attention_rnn_dim)
        ...
        self.decoder_rnn = nn.LSTMCell(
            self.hidden_cat_dim + attention_rnn_dim,
            decoder_rnn_dim)

        self.linear_projection = LinearNorm(
            self.hidden_cat_dim + decoder_rnn_dim,
            n_mel_channels * n_frames_per_step_decoder)

        self.stop_layer = LinearNorm(
            self.hidden_cat_dim + decoder_rnn_dim, 1,
